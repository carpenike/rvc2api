...
export function CanSniffer() {
  ...
  return (
    <main className="p-4 md:p-8 bg-background min-h-screen" aria-label="CAN Sniffer page">
      <Card className="max-w-6xl mx-auto bg-card shadow-md rounded-lg p-4 md:p-6">
        <header className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between">
          <h1 className="text-2xl font-bold text-foreground mb-2 md:mb-0" id="can-sniffer-heading">
            CAN Sniffer
          </h1>
        ...
        <div className="mb-4 flex flex-col md:flex-row md:items-center gap-2">
          <label htmlFor="can-filter" className="text-foreground font-medium">
            Filter:
          </label>
          <input
            id="can-filter"
            ...
            className="bg-background border border-border rounded-md px-2 py-1 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
        ...
        {loading ? (
          <div className="flex items-center justify-center py-8" role="status" aria-live="polite" data-testid="loading-state">
            <span className="text-foreground">Loading CAN messages…</span>
          </div>
        ) : error ? (
          ...
          <section aria-labelledby="can-sniffer-heading">
            <div className="overflow-x-auto rounded-md border border-border bg-card">
              <table className="min-w-full text-sm text-foreground" aria-label="CAN messages table" data-testid="can-table">
                ...
                <tbody>
                  {filteredMessages.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="text-center text-muted-foreground py-6" data-testid="no-messages-row">
                    ...
                    filteredMessages.map((msg, i) => (
                      <tr
                        key={i}
                        className={clsx(i % 2 === 0 ? "bg-card" : "bg-muted/50")}
                        data-testid="can-row"
                      >
                        <td className="px-2 py-1 whitespace-nowrap">{msg.time}</td>
                        <td className={clsx("px-2 py-1 font-mono", msg.dir === "TX" ? "text-primary" : "text-secondary")}>{msg.dir}</td>
        ...
        <div className="mt-4 flex justify-between items-center text-xs text-muted-foreground">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              className="bg-background border border-border text-foreground rounded-lg px-2 py-1 text-xs"
            >
