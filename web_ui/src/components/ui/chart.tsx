                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <div className="text-muted-foreground">
                        {itemConfig?.label || (typeof item.name === "string" ? item.name : "")}
                      </div>
                    </div>
                    {item.value != null && (
...
function ChartLegend({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }: React.ComponentProps<typeof RechartsPrimitive.Legend> & {
  hideIcon?: boolean;
  nameKey?: string;
}) {
  const { config } = useChart();
...
function ChartLegendContent({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }: React.ComponentProps<typeof RechartsPrimitive.Legend> & {
  hideIcon?: boolean;
  nameKey?: string;
}) {
  const { config } = useChart();
